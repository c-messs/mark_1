 ...
 *  - Generate a fileset per issuer
 *  - Can generate 100,000+ 2 member policies per file.
 *
 */
public class TestSbmFileGenerator2 extends AbstractTestSbmFileGenerator implements Runnable {


	/**
	 * Number of files per Issuer, max 93,000.
	 */
	 ...
	 */
	private static final Long POL_LEN = 7L;
	/**
	...
	public static void main(String[] args) {
		...
		Thread t = new Thread(new TestSbmFileGenerator2());
		t.start();	
		System.out.println("TestSbmFileGenerator 2 (fileset with multiple files for a single issuer): STARTED\n - - - - - - - - - - - - - - - ");
	}
	...
	public void run() {
		...
		// For "multiple files (one per issuer)",
		int issuerIdInt = 10000;
		...
		for (int s = 0; s < STATE_LEN; ++s) {
			...
			for (int i = 0; i < ISSUER_LEN; ++i) {
				...
				fileSetId = null;
				// Reset file set attributes (per Issuer)
				fileNum = 1;

				// Number of files per issuer
				for(int f = 0; f < FILE_LEN; ++f) {
					...
					fileSetId = issuerId + String.format("%03d", (f + 1));

					enrollment = TestDataSBMUtility.makeEnrollment(fileId, tenantId, covYr, issuerId, TestDataSBMUtility.FILES_FILESET);
					// MakeEnrollment will make fileInfo and IssuerFileInfo
					issuerFileSet = TestDataSBMUtility.makeIssuerFileSet(fileSetId, fileNum, FILE_LEN);
					enrollment.getFileInformation().getIssuerFileInformation().setIssuerFileSet(issuerFileSet);
					
					String sbmFileId = enrollment.getFileInformation().getFileId();
					String issuerFileSetId = enrollment.getFileInformation().getIssuerFileInformation().getIssuerFileSet().getIssuerFileSetId();
					updateStatusFileLines.add((i + 1) + "," + tenantId + "," + issuerId + "," + sbmFileId + "," + issuerFileSetId + ",Approve");

					for (int q = 0; q < QHPID_LEN; ++q) {

						qhpId = issuerId + states[f] +  String.format("%04d", (q + 1)) + variantId;
		...
		System.out.println(" - - - - - - - - - - - - - - - - \nTestSbmFileGenerator 2: COMPLETED");
	}
