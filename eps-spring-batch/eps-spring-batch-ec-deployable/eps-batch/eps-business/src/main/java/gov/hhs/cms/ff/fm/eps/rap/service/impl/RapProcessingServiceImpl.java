...
/**
 * This is the main service class for the Retro Active Payment Processing. It
 * accepts the Policy version as input, compares the Payments that were made to
 * the Payments that should have been made and creates Retroactive Payment
 * transactions
 * 
...
public class RapProcessingServiceImpl implements RapProcessingService {
	...
	private RapProcessServiceHandler rapHandler;
	 ...
	 * It accepts the Policy version as input, compares the Payments that were
	 * made to the Payments that should have been made and creates Retroactive
	...
	public RAPProcessingResponse processRetroActivePayments(RAPProcessingRequest request)
		...
		currentPmtMonth = rapHandler.getPaymentMonthERC(codeDecodesHelper);
		...
		// Evaluate the Payment Transactions and create Retro Active Payments
		// for the policy version
		List<PolicyPaymentTransDTO> retroActivePayments = createRetroActivePaymentTxns(policyVersion,
	...
	/*
	 * Determine whether the state is FFM prorating for the Payment Transaction
	 * year
	 */
	 ...
	 */
	private List<PolicyPaymentTransDTO> createRetroActivePaymentTxns(PolicyDataDTO policyVersion,
			PolicyDetailDTO policyDetails) {
		...
		Map<DateTime, List<PolicyPremium>> premiumsForMonths = getPremiumsForCoverageMonths(policyPremiums,
				policyVersion);
		...
		for (DateTime coverageDate : minMaxCoverageMonths) {
			ProrationType prorationType = RapProcessingHelper.getProrationType(policyVersion.getSubscriberStateCd(),
			...
			// use only the earliest premium record for the month as basis for
			// creating payments
			// ie, only one payment for the month based on the oldest effective
			// premium for the month
			if ((!prorationType.equals(ProrationType.SBM_PRORATING)
					&& !isStateFfmProrating(policyVersion.getSubscriberStateCd(), coverageDate.getYear()))
			...
			// if no payments exists for coverage date then create retros for
			// all three programTypes
			ctrateRetros(payments, premiumRecs, coverageDate, policyVersion, prorationType, pmtTransList);
		}
		LOGGER.info("EXIT createRetroActivePaymentTxns()");
		return pmtTransList;
	}
	private void ctrateRetros(List<PolicyPaymentTransDTO> payments, List<PolicyPremium> premiumRecs,
			DateTime coverageDate, PolicyDataDTO policyVersion, ProrationType prorationType,
		...
		if (CollectionUtils.isEmpty(payments)) {
			...
			if (!prorationType.equals(ProrationType.SBM_PRORATING)
					&& rapHandler.isUserFeeRateExists(coverageDate, policyVersion, rapDao)) {
	 ...
	 */
	private void reversePmtsPriorToPolicyStartDate(List<PolicyPaymentTransDTO> pmtTransList,
			Map<DateTime, List<PolicyPaymentTransDTO>> paymentsForMonths, PolicyDataDTO policyVersion) {
				...
				if (coverageDt.isBefore(minPaymentStart)) {
					...
					for (PolicyPaymentTransDTO pmt : payments) {
						if (!reversalRefIds.contains(pmt.getPolicyPaymentTransId())) {
							createReversalForCancels(pmtTransList, coverageDt, pmt.getFinancialProgramTypeCd(),
									policyVersion, pmt);
						}
					}
				} else {
	 ...
	 */
	private void createAptcPayments(PolicyDataDTO policyVersion, DateTime coverageDate,
			List<PolicyPaymentTransDTO> pmtTransList, List<PolicyPaymentTransDTO> payments,
			List<PolicyPremium> premiumRecs, ProrationType prorationType) {
		...
		if (premiumRecs != null) {
			...
			manipulateProrate(premiums, coverageDate, aptcPayments, policyVersion, prorationType, pmtTransList);
		}
		// Future Cancel
	...
	private void manipulateProrate(List<PolicyPremium> premiums, DateTime coverageDate,
			List<PolicyPaymentTransDTO> aptcPayments, PolicyDataDTO policyVersion, ProrationType prorationType,
			List<PolicyPaymentTransDTO> pmtTransList) {
		...
		for (PolicyPremium policyPremium : premiums) {
			BigDecimal epsAptc = policyPremium.getAptcAmount() == null ? BigDecimal.ZERO
					: policyPremium.getAptcAmount();
			DateTime coverageMonthEndDate = coverageDate.dayOfMonth().withMaximumValue();
			DateTime premiumStartDate = getPremiumStrtDte(policyPremium, coverageDate);
			DateTime premiumEndDate = getPremiumEndDte(policyPremium, coverageMonthEndDate);
			PolicyPaymentTransDTO pmt = getPaymentForPremiumPeriod(aptcPayments, premiumStartDate, premiumEndDate);
			...
			if (pmt != null) {
				...
				if (!(pmt.getPaymentAmount().compareTo(proratedAmount) == 0)) {
					createAdjustmentForMatchingDates(tempHolder, policyVersion, pmt, policyPremium, epsAptc,
							policyPremium.getProratedAptcAmount(), prorationType);
				} else {
				...
				for (PolicyPaymentTransDTO payment : aptcPayments) {
					createReversalForNonMatchingDates(pmtTransList, coverageDate, premiumStartDate, APTC, policyVersion,
							payment);
				}
				if (!policyVersion.isPolicyCancelled() && !coverageDate.isAfter(premiumEndDate)
						&& epsAptc.compareTo(BigDecimal.ZERO) != 0) {
					pmtTransList.add(createPolicyPaymentTrans(coverageDate, APTC, policyVersion, policyPremium, epsAptc,
							policyPremium.getProratedAptcAmount(), prorationType));
				}
			}
		}
	}
	private DateTime getPremiumEndDte(PolicyPremium policyPremium, DateTime coverageMonthEndDate) {
		if (policyPremium.getEffectiveEndDate() == null
				|| policyPremium.getEffectiveEndDate().isAfter(coverageMonthEndDate)) {
			return coverageMonthEndDate;
		}
		return policyPremium.getEffectiveEndDate();
	}
	private DateTime getPremiumStrtDte(PolicyPremium policyPremium, DateTime coverageDate) {
		if (policyPremium.getEffectiveStartDate().isBefore(coverageDate)) {
			return coverageDate;
		}
		return policyPremium.getEffectiveStartDate();
	}
	/*
	...
	private void createCsrPayments(PolicyDataDTO policyVersion, DateTime coverageDate,
		...
		if (premiumRecs != null) {
			List<PolicyPremium> premiums = new ArrayList<PolicyPremium>();
			premiums.addAll(premiumRecs);
			clearPrem(premiums, prorationType);
			for (PolicyPremium policyPremium : premiums) {
				DateTime coverageMonthEndDate = coverageDate.dayOfMonth().withMaximumValue();
				BigDecimal epsCsr = getEpcCsrAmt(policyPremium);
				DateTime premiumStartDate = getPremiumStrtDte(policyPremium, coverageDate);
				DateTime premiumEndDate = getPremiumEndDte(policyPremium, coverageMonthEndDate);
				PolicyPaymentTransDTO pmt = getPaymentForPremiumPeriod(csrPayments, premiumStartDate, premiumEndDate);
				LOGGER.debug(premiumStartDate + " to " + premiumEndDate);
				// Premium Dates match existing Payment dates
				if (pmt != null) {
					BigDecimal proratedAmount = getProratedAmount(epsCsr, policyPremium.getProratedCsrAmount(),
							...
							prorationType);
					if (!(pmt.getPaymentAmount().compareTo(proratedAmount) == 0)) {
						createAdjustmentForMatchingDates(tempHolder, policyVersion, pmt, policyPremium, epsCsr,
								policyPremium.getProratedCsrAmount(), prorationType);
					} else {
					...
					for (PolicyPaymentTransDTO payment : csrPayments) {
						createReversalForNonMatchingDates(pmtTransList, coverageDate, premiumStartDate, CSR,
								policyVersion, payment);
					}
					// Create the new Adjustment Transaction
					if (adjustmentNeeded(policyVersion, coverageDate, epsCsr, premiumEndDate)) {
						pmtTransList.add(createPolicyPaymentTrans(coverageDate, CSR, policyVersion, policyPremium,
								epsCsr, policyPremium.getProratedCsrAmount(), prorationType));
					}
				}
			}
		}
	...
	private boolean adjustmentNeeded(PolicyDataDTO policyVersion, DateTime coverageDate, BigDecimal epsCsr,
			DateTime premiumEndDate) {
		if (!policyVersion.isPolicyCancelled() && !coverageDate.isAfter(premiumEndDate)
				&& epsCsr.compareTo(BigDecimal.ZERO) != 0) {
			return true;
		}
		return false;
	}
	private void clearPrem(List<PolicyPremium> premiums, ProrationType prorationType) {
		if (prorationType.equals(ProrationType.SBM_PRORATING) && RapProcessingHelper.hasNoProratedCsr(premiums)) {
			premiums.subList(1, premiums.size()).clear();
		}
	}
	private BigDecimal getEpcCsrAmt(PolicyPremium policyPremium) {
		if (policyPremium.getCsrAmount() == null) {
			return BigDecimal.ZERO;
		}
		return policyPremium.getCsrAmount();
	}
	/*
	...
	private void createUfPayments(PolicyDataDTO policyVersion, DateTime coverageDate,
		...
		if (premiumRecs != null) {
			for (PolicyPremium policyPremium : premiumRecs) {
				BigDecimal epsUf = makeEpsf(policyPremium);
				DateTime coverageMonthEndDate = coverageDate.dayOfMonth().withMaximumValue();
				DateTime premiumStartDate = makeEffectDate(policyPremium, coverageDate);
				DateTime premiumEndDate = (policyPremium.getEffectiveEndDate() == null
						|| policyPremium.getEffectiveEndDate().isAfter(coverageMonthEndDate)) ? coverageMonthEndDate
								: policyPremium.getEffectiveEndDate();
				PolicyPaymentTransDTO pmt = getPaymentForPremiumPeriod(ufPayments, premiumStartDate, premiumEndDate);
				LOGGER.debug(premiumStartDate + " to " + premiumEndDate);
				// Premium Dates match existing Payment dates
				if (pmt != null) {
					...
					addToTransList(pmt, proratedAmount, pmtTransList, policyVersion, policyPremium, epsUf,
							coverageDate);
				} else {
					// Premium dates differ from existing payment dates
					for (PolicyPaymentTransDTO payment : ufPayments) {
						createReversalForNonMatchingDates(pmtTransList, coverageDate, premiumStartDate, UF,
								policyVersion, payment);
					}
					// Create the new Adjustment Transaction
					if (creatingNewAdjustmentTwo(epsUf, coverageDate, policyVersion, premiumEndDate)) {
						pmtTransList.add(createPolicyPaymentTrans(coverageDate, UF, policyVersion, policyPremium, epsUf,
								policyPremium.getProratedPremiumAmount(), ProrationType.NON_PRORATING));
					}
				}
			}
		}
	...
	private DateTime makeEffectDate(PolicyPremium policyPremium, DateTime coverageDate) {
		if (policyPremium.getEffectiveStartDate().isBefore(coverageDate)) {
			return coverageDate;
		}
		return policyPremium.getEffectiveStartDate();
	}
	private BigDecimal makeEpsf(PolicyPremium policyPremium) {
		if (policyPremium.getTotalPremiumAmount() == null) {
			return BigDecimal.ZERO;
		}
		return policyPremium.getTotalPremiumAmount();
	}
	private void addToTransList(PolicyPaymentTransDTO pmt, BigDecimal proratedAmount,
			List<PolicyPaymentTransDTO> pmtTransList, PolicyDataDTO policyVersion, PolicyPremium policyPremium,
			BigDecimal epsUf, DateTime coverageDate) {
		if (!(pmt.getTotalPremiumAmount().compareTo(proratedAmount) == 0)) {
			...
			// Create the new Adjustment Transaction
			if (creatingNewAdjustment(epsUf, coverageDate, policyVersion)) {
				pmtTransList.add(createPolicyPaymentTrans(coverageDate, UF, policyVersion, policyPremium, epsUf,
						policyPremium.getProratedPremiumAmount(), ProrationType.NON_PRORATING));
			}
		} else {
	...
	private boolean creatingNewAdjustmentTwo(BigDecimal epsUf, DateTime coverageDate, PolicyDataDTO policyVersion,
			DateTime premiumEndDate) {
		if (!policyVersion.isPolicyCancelled() && !coverageDate.isAfter(premiumEndDate)
				&& epsUf.compareTo(BigDecimal.ZERO) != 0) {
			return true;
		}
		return false;
	}
	private boolean creatingNewAdjustment(BigDecimal epsUf, DateTime coverageDate, PolicyDataDTO policyVersion) {
	 ...
	 */
	private void createReversalForNonMatchingDates(List<PolicyPaymentTransDTO> pmtTransList, DateTime coverageDate,
			DateTime premiumStartDate, String programType, PolicyDataDTO policyVersion, PolicyPaymentTransDTO payment) {
		if (!premiumStartDate.isAfter(payment.getPaymentCoverageEndDate())) {
			...
			if (payment.getLastPaymentProcStatusTypeCd().equalsIgnoreCase(STATUS_PENDING_CYCLE)) {
				...
				if (!payment.getLastPaymentProcStatusTypeCd().equalsIgnoreCase(STATUS_REPLACED)
					...
					pmtTransList.add(createReversalPaymentTrans(coverageDate, programType, policyVersion, payment,
							reversalRefId));
					payment.setParentPolicyPaymentTransId(reversalRefId);
				}
			}
		} else {
			if (policyVersion.isPolicyCancelled() || coverageDate.isAfter(policyVersion.getPolicyEndDate())) {
	 ...
	 */
	private void createReversalForCancels(List<PolicyPaymentTransDTO> pmtTransList, DateTime coverageDate,
		...
		if (policyVersion.isPolicyCancelled()
			...
			if (payment.getLastPaymentProcStatusTypeCd().equalsIgnoreCase(STATUS_PENDING_CYCLE)) {
				...
				reversalRefId = payment.getPolicyPaymentTransId();
				pmtTransList.add(
						createReversalPaymentTrans(coverageDate, programType, policyVersion, payment, reversalRefId));
			}
		}
	}
	/*
	 ...
	 */
	private void createAdjustmentForMatchingDates(List<Object> tempHolder, PolicyDataDTO policyVersion,
		...
		List<PolicyPaymentTransDTO> pmtTransList = (List<PolicyPaymentTransDTO>) tempHolder.get(0);
		DateTime coverageDate = (DateTime) tempHolder.get(1);
		String programType = (String) tempHolder.get(2);
		...
		if (payment.getLastPaymentProcStatusTypeCd().equalsIgnoreCase(STATUS_PENDING_CYCLE)) {
			...
			reversalRefId = payment.getPolicyPaymentTransId();
			pmtTransList
					.add(createReversalPaymentTrans(coverageDate, programType, policyVersion, payment, reversalRefId));
		}
		if (epsAmount.compareTo(BigDecimal.ZERO) != 0
				&& !payment.getPaymentCoverageStartDate().isAfter(policyVersion.getPolicyEndDate())
				&& !policyVersion.isPolicyCancelled()) {
			pmtTransList.add(createPolicyPaymentTrans(coverageDate, programType, policyVersion, policyPremium,
					epsAmount, proratedAmount, prorationType));
		}
	}
	/*
	 * Get Prorated Amount
	 */
	private BigDecimal getProratedAmount(BigDecimal paymentAmount, BigDecimal proratedPmtAmount, DateTime coverageDate,
			DateTime effectiveStartDate, DateTime premiumEndDate, String stateCd, ProrationType prorationType) {
		if (!prorationType.equals(ProrationType.SBM_PRORATING)
				&& !isStateFfmProrating(stateCd, coverageDate.getYear())) {
		...
		int prorationDays = RapProcessingHelper.getProrationDaysOfCoverage(coverageDate, effectiveStartDate,
				premiumEndDate);
		int totalDaysInCoverageMonth = coverageDate.dayOfMonth().getMaximumValue();
		BigDecimal proratedAmount = RapProcessingHelper.getProratedAmount(paymentAmount, prorationDays,
				totalDaysInCoverageMonth);
		return proratedAmount;
	}
	/*
	 * Get Payment record corresponding to Premium Period
	 */
	private PolicyPaymentTransDTO getPaymentForPremiumPeriod(List<PolicyPaymentTransDTO> payments,
			DateTime effectiveStartDate, DateTime premiumEndDate) {
		for (PolicyPaymentTransDTO pmt : payments) {
			if (pmt.getPaymentCoverageStartDate().equals(effectiveStartDate)
					&& pmt.getPaymentCoverageEndDate().equals(premiumEndDate)
	 ...
	 */
	private List<PolicyPaymentTransDTO> getProgramSpecificPayments(List<PolicyPaymentTransDTO> payments,
			String financialPgmType) {
	 ...
	 */
	private Map<DateTime, List<PolicyPremium>> getPremiumsForCoverageMonths(List<PolicyPremium> policyPremiums,
		...
		if (policyPremiums != null) {
			for (PolicyPremium policyPremium : policyPremiums) {
				...
				DateTime premiumEndDate = policyPremium.getEffectiveEndDate() == null ? new DateTime(HIGH_DATE)
						: policyPremium.getEffectiveEndDate();
				if (premiumEndDate.isBefore(policyVersion.getPolicyStartDate())) {
					continue;
				}
				addingPolidyPremium(premiumsForMonths, covrageDate, policyPremium);
				if (policyPremium.getEffectiveEndDate() == null
		...
		performUpdateNotEmptyMap(premiumsForMonths);
		return premiumsForMonths;
	}
	private void addingPolidyPremium(Map<DateTime, List<PolicyPremium>> premiumsForMonths, DateTime covrageDate,
		...
		if (premiumsForMonths.containsKey(covrageDate)) {
			...
			premiumsForCovrageMonths = new ArrayList<>();
			premiumsForCovrageMonths.add(policyPremium);
			premiumsForMonths.put(covrageDate, premiumsForCovrageMonths);
		}
	}
	private void performUpdateNotEmptyMap(Map<DateTime, List<PolicyPremium>> premiumsForMonths) {
		if (!premiumsForMonths.isEmpty()) {
			...
			for (DateTime coverageMonth : minMaxCoverageMonths) {
				if (premiumsForMonths.containsKey(coverageMonth)) {
					prevPolicyPremium = premiumsForMonths.get(coverageMonth)
							.get(premiumsForMonths.get(coverageMonth).size() - 1);
				} else {
	 ...
	 */
	private Map<DateTime, List<PolicyPaymentTransDTO>> getPaymentsForCoverageMonths(
			List<PolicyPaymentTransDTO> policyPayments) {
	...
	private List<DateTime> determineMinMaxCoverageMonth(PolicyDataDTO policyVersion) {
		...
		if (minPaymentMonth.getYear() < issuerTransitionDt.getYear()) {
			minPaymentMonth = new DateTime().withYear(issuerTransitionDt.getYear()).withDayOfYear(1)
					.withTimeAtStartOfDay();
			LOGGER.info(
					"Policy start year less than issuer Transition year. Setting Min Covg Month to Jan of Issuer transition Year "
							+ minPaymentMonth);
		}
	 ...
	 */
	private List<PolicyPaymentTransDTO> createRetros(DateTime coverageDate, PolicyDataDTO policy,
			List<PolicyPremium> premiumRecs, ProrationType prorationType) {
		List<PolicyPaymentTransDTO> transactionList = new ArrayList<PolicyPaymentTransDTO>();
		for (PolicyPremium premium : premiumRecs) {
			DateTime premiumEndDate = premium.getEffectiveEndDate() == null ? new DateTime(HIGH_DATE)
					: premium.getEffectiveEndDate();
			if (!premiumEndDate.isBefore(coverageDate) && !policy.isPolicyCancelled()) {
				addToTransList(coverageDate, policy, premium, prorationType, transactionList);
			}
		}
		return transactionList;
	}
	private void addToTransList(DateTime coverageDate, PolicyDataDTO policy, PolicyPremium premium,
			ProrationType prorationType, List<PolicyPaymentTransDTO> transactionList) {
		...
		if (aptcAmount != null && aptcAmount.compareTo(BigDecimal.ZERO) != 0) {
			transactionList.add(createPolicyPaymentTrans(coverageDate, APTC, policy, premium, premium.getAptcAmount(),
					premium.getProratedAptcAmount(), prorationType));
		}
		BigDecimal csrAmount = premium.getCsrAmount();
		if (csrAmount != null && csrAmount.compareTo(BigDecimal.ZERO) != 0) {
			transactionList.add(createPolicyPaymentTrans(coverageDate, CSR, policy, premium, premium.getCsrAmount(),
					premium.getProratedCsrAmount(), prorationType));
		}
		...
		if (totalPremiumAmount != null && totalPremiumAmount.compareTo(BigDecimal.ZERO) != 0) {
			if (!prorationType.equals(ProrationType.SBM_PRORATING)
					&& rapHandler.isUserFeeRateExists(coverageDate, policy, rapDao)) {
				transactionList.add(
						createPolicyPaymentTrans(coverageDate, UF, policy, premium, premium.getTotalPremiumAmount(),
								premium.getProratedPremiumAmount(), ProrationType.NON_PRORATING));
			}
		}
	}
	/*
	 ...
	 */
	private List<PolicyPaymentTransDTO> createRetrosForSbm(DateTime coverageDate, PolicyDataDTO policy,
			List<PolicyPremium> premiumRecs, ProrationType prorationType) {
		List<PolicyPaymentTransDTO> transactionList = new ArrayList<PolicyPaymentTransDTO>();
		// APTC
		List<PolicyPremium> premiumsNoProratedAptc = new ArrayList<PolicyPremium>();
		premiumsNoProratedAptc.addAll(premiumRecs);
		manipulateAptc(premiumsNoProratedAptc, coverageDate, transactionList, policy, prorationType);
		// CSR
		...
		for (PolicyPremium premium : premiumsNoProratedCsr) {
			...
			if (!premiumEndDate.isBefore(coverageDate) && !policy.isPolicyCancelled()) {
				BigDecimal csrAmount = premium.getCsrAmount();
				if (csrAmount != null && csrAmount.compareTo(BigDecimal.ZERO) != 0) {
					transactionList.add(createPolicyPaymentTrans(coverageDate, CSR, policy, premium,
							premium.getCsrAmount(), premium.getProratedCsrAmount(), prorationType));
				}
			}
		}
		return transactionList;
	}
	private void manipulateAptc(List<PolicyPremium> premiumsNoProratedAptc, DateTime coverageDate,
			List<PolicyPaymentTransDTO> transactionList, PolicyDataDTO policy, ProrationType prorationType) {
		...
		for (PolicyPremium premium : premiumsNoProratedAptc) {
			...
			if (!premiumEndDate.isBefore(coverageDate) && !policy.isPolicyCancelled()) {
				BigDecimal aptcAmount = premium.getAptcAmount();
				if (aptcAmount != null && aptcAmount.compareTo(BigDecimal.ZERO) != 0) {
					transactionList.add(createPolicyPaymentTrans(coverageDate, APTC, policy, premium,
							premium.getAptcAmount(), premium.getProratedAptcAmount(), prorationType));
				}
			}
		}
	}
	 ...
	 */
	private PolicyPaymentTransDTO createPolicyPaymentTrans(DateTime coverageMonth, String financialProgramTypeCd,
			PolicyDataDTO policy, PolicyPremium premium, BigDecimal amount, BigDecimal proratedAmount,
			ProrationType prorationType) {
		...
		dto = updatePaymentDto(dto, coverageStartDate, coverageMonth, premium);
		BigDecimal totalPremiumAmount = premium.getTotalPremiumAmount();
		...
		if (isStateFfmProrating(policy.getSubscriberStateCd(), coverageMonth.getYear())) {
			prorationDays = RapProcessingHelper.getProrationDaysOfCoverage(coverageMonth,
					dto.getPaymentCoverageStartDate(), dto.getPaymentCoverageEndDate());
			...
			if (premium.getProratedPremiumAmount() == null) {
				totalPremiumAmount = RapProcessingHelper.getProratedAmount(totalPremiumAmount, prorationDays,
						daysInCoverageMonth);
			} else {
	...
	private PolicyPaymentTransDTO updatePaymentDto(PolicyPaymentTransDTO dto, DateTime coverageStartDate,
			DateTime coverageMonth, PolicyPremium premium) {
		...
		DateTime coverageEndDate = premium.getEffectiveEndDate() == null ? new DateTime(HIGH_DATE)
				: premium.getEffectiveEndDate();
	 ...
	 */
	private PolicyPaymentTransDTO createReversalPaymentTrans(DateTime coverageDate, String financialProgramTypeCd,
			PolicyDataDTO policy, PolicyPaymentTransDTO paymentTrans, Long reversalRefId) {
	 ...
	 */
	private void createReplacementTrans(List<PolicyPaymentTransDTO> pmtTransList, PolicyPaymentTransDTO pmt,
			PolicyDataDTO policy) {
