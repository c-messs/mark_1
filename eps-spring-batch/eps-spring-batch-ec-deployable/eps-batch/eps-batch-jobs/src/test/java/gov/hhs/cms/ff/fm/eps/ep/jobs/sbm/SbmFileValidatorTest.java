...
public class SbmFileValidatorTest extends BaseBatchSBMTest{
	...
	public void testTotalIssuerFileCount_noError() {
       ...
       @Test
       public void testCoveragYear_ER_015() {
             
             String expectedErrCd = SBMErrorWarningCode.ER_015.getCode();
             
             SBMFileProcessingDTO dto = createSBMFileProcessingDTO();
             LocalDate runDate = DEC_1.minusYears(1);
             int coverageYear = YEAR;
             dto.getFileInfoType().setCoverageYear(coverageYear);
             
             sbmFileValidator.setCoverageYear(coverageYear+1);
             
       
             ReflectionTestUtils.invokeMethod(sbmFileValidator, "validateCoverageYear", dto, runDate);
             
             assertEquals("ErrorList size", 1, dto.getErrorList().size());
             SBMErrorDTO actual = dto.getErrorList().get(0);
             assertEquals("SBMErrorDTO", expectedErrCd, actual.getSbmErrorWarningTypeCd());
       }

       @Test
       public void testCoveragYear_ER_017() {
             
             String expectedErrCd = SBMErrorWarningCode.ER_017.getCode();
             
             SBMFileProcessingDTO dto = createSBMFileProcessingDTO();
             LocalDate runDate = APR_1;
             int coverageYear = YEAR+1;
             dto.getFileInfoType().setCoverageYear(coverageYear);
       ...
       @Test
       public void testCoveragYear_ER_016() {
             
             String expectedErrCd = SBMErrorWarningCode.ER_016.getCode();
             
             SBMFileProcessingDTO dto = createSBMFileProcessingDTO();
             LocalDate runDate = LocalDate.of(LocalDate.now().getYear()-1, 1, 1);
             int coverageYear = YEAR;
             dto.getFileInfoType().setCoverageYear(coverageYear+1);
