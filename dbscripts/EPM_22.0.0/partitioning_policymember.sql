...
as select * from policymember where 1=2;

pause 'Verify if can redefine - hit return: '
exec dbms_redefinition.can_redef_table('EPS_OWNER','POLICYMEMBER',dbms_redefinition.cons_use_pk);

pause 'Start redefinition and copy data - hit return: '
EXEC DBMS_REDEFINITION.START_REDEF_TABLE('EPS_OWNER', 'POLICYMEMBER','POLICYMEMBER_T',null,dbms_redefinition.cons_use_pk,null,null)

pause 'Sync data from snapshot
exec DBMS_REDEFINITION.SYNC_INTERIM_TABLE(uname=>user, orig_table => 'POLICYMEMBER',int_table=>'POLICYMEMBER_T')
pause 'Check log status of snapshot log - hit return: '
select log_table from user_snapshot_logs where master = 'POLICYMEMBER';
...
DBMS_REDEFINITION.COPY_TABLE_DEPENDENTS(
    uname               => user,
    orig_table          => 'POLICYMEMBER',
    int_table           => 'POLICYMEMBER_T',
...
Select table_name, partitioned from user_tables where table_name in ('POLICYMEMBER','POLICYMEMBER_T');
select * from dba_redefinition_errors;

pause 'finish redefinition - hit return: '
BEGIN
DBMS_REDEFINITION.FINISH_REDEF_TABLE(user, 'POLICYMEMBER', 'POLICYMEMBER_T');
END;
/
