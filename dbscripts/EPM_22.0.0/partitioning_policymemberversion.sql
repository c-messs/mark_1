...
create table POLICYMEMBERVERSION_T
...
as select * from POLICYMEMBERVERSION where 1=2;

pause 'Verify if can redefine - hit return: '
exec dbms_redefinition.can_redef_table('EPS_OWNER','POLICYMEMBERVERSION',dbms_redefinition.cons_use_pk);

pause 'Start redefinition and copy data - hit return: '
EXEC DBMS_REDEFINITION.START_REDEF_TABLE('EPS_OWNER', 'POLICYMEMBERVERSION','POLICYMEMBERVERSION_T',null,dbms_redefinition.cons_use_pk,null,null)

pause 'Sync data from snapshot
exec DBMS_REDEFINITION.SYNC_INTERIM_TABLE(uname=>user, orig_table => 'POLICYMEMBERVERSION',int_table=>'POLICYMEMBERVERSION_T')
pause 'Check table status and redefinition errors  DO NOT hit return until errors are fixed: '
select log_table from user_snapshot_logs where master = 'POLICYMEMBERVERSION';
...
DBMS_REDEFINITION.COPY_TABLE_DEPENDENTS(
    uname               => user,
    orig_table          => 'POLICYMEMBERVERSION',
    int_table           => 'POLICYMEMBERVERSION_T',
...
END;
/
pause 'Check table status - hit return: '
Select table_name, partitioned from user_tables where table_name in ('POLICYMEMBERVERSION','POLICYMEMBERVERSION_T');

pause 'finish redefinition - hit return: '
BEGIN
DBMS_REDEFINITION.FINISH_REDEF_TABLE(user, 'POLICYMEMBERVERSION', 'POLICYMEMBERVERSION_T');
END;
/
